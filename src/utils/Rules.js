...
class SmallStraight extends Rule {
    evalRoll = dice => {
        ...
        const sequence = d
            .map((val, i) => {
                const validate = prevVal + 1 === val;
                prevVal = val;
                return validate;
            })
            .splice(1);
        let isValid = sequence.every(el => el === true);
        if (d.length > 4 && !isValid) {
            isValid =
                sequence.slice(1).every(el => el === true) ||
                sequence
                    .reverse()
                    .slice(1)
                    .every(el => el === true);
        ...
        return isValid && d.length >= 4 ? this.score : 0;
    };
}
